...
if(isset($_POST['add_server'])) {
	...
	if(!Server::hostname_valid($hostname)) {
		...
		if(count($admins) == count($admin_names)) {
			...
			try {
				...
				$alert = new UserAlert;
				$alert->content = 'Server \'<a href="'.rrurl('/servers/'.urlencode($hostname)).'" class="ska-alert-link">'.hesc($hostname).'</a>\' successfully created.';
				$alert->escaping = ESC_NONE;
				$active_user->add_alert($alert);
			} catch(ServerAlreadyExistsException $e) {
				$alert = new UserAlert;
				$alert->content = 'Server \'<a href="'.rrurl('/servers/'.urlencode($hostname)).'" class="ska-alert-link">'.hesc($hostname).'</a>\' is already known by SSH Key Authority.';
				$alert->escaping = ESC_NONE;
				$alert->class = 'danger';
				$active_user->add_alert($alert);
	...
	if ($entries !== null) {
		...
		if ($result['existed'] > 0) {
			if ($result['existed'] == 1) {
				$msg .= ", 1 server is already known by SSH Key Authority";
			} else {
				$msg .= ", {$result['existed']} servers are already known by SSH Key Authority";
			}
		}
		$alert->content = $msg;
	} else {
