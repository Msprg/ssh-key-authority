...
- Bootstrap handoff prep via repo-local aliases such as `ska-btn*`, `ska-alert*`, `ska-form-label`, `ska-form-control`, `ska-input-group*`, `ska-form-check*`, and `ska-badge*`, reducing future class-family conflicts when real Bootstrap 5 CSS is introduced
...
- continue migrating locally owned Bootstrap-named class families onto explicit `ska-*` aliases, with shared `form-control`, alert/button variants, and grid helpers next in line
