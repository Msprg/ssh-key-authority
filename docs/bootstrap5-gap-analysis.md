...
- Bootstrap handoff prep has started: repo-local aliases such as `ska-btn*`, `ska-alert*`, `ska-form-label`, `ska-form-control`, `ska-input-group*`, `ska-form-check*`, and `ska-badge*` now exist and are in use on the shell/login plus the busiest list/detail pages, with the main list/detail forms, actions, and alerts now migrated to `ska-form-control`, `ska-btn*`, and `ska-alert*`.
...
| --- | --- | --- | --- | --- |
| Global shell | `templates/base.php` | Bootstrap 5-ready | Native dropdown and alert-dismiss behavior; no Bootstrap or jQuery runtime assets; shell container/text utility classes are repo-local | Residual risk is visual parity on less-traveled shell states, not framework dependency |
